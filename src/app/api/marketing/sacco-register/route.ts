...
export async function POST(req: NextRequest) {
  ...
  if (!apiKey) {
    return NextResponse.json(
      { message: 'Marketing registration is not configured (MARKETING_API_KEY).' },
      { status: 503 }
    );
  }
  ...
  const upstreamUrl = `${upstreamBase}/api/marketing/sacco-register`;
  ...
  try {
    ...
    console.error('Marketing sacco-register:', upstreamUrl, err);
    return NextResponse.json(
      { message: 'Unable to reach registration service. Try again later.' },
      { status: 502 }
    );
  }
